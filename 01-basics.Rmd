...
```{r}
knitr::include_graphics('img/scipro.png')
```
...
If you're attempting applied science, your workflow may look something like this.  

```{r}
knitr::include_graphics('img/appscipro.png')
```

Many scientists, especially early career researchers (my past self included), may assume that this is sufficient to affect change. We write the report, sent it out into the world, and move on to the next project. This is a common mentality:

> "This 500-page report will answer all of their questions!"

From the other side, such as the manager or administor, the report may be received like this: 
...
As I'm sure we're aware, the environmental management community is often siloed with each branch doing their own thing and speaking their own languages.  Between the research (and often academic) and management community, we call this the research-management divide. 

```{r}
knitr::include_graphics('img/divide2.png')
```

A distinct barrier exists between how scientific products are developed and how they fail to meet management needs. This is often the result of communication barriers, irreproducible results, information loss with irreproducible results, inaccessible data, and opaque workflows. 

These barriers often begin at the publication level of research.  This compelling graphic from @Michener97 describes the atrophy of information in a closed approach to creating science.

```{r}
knitr::include_graphics('img/michener.png')
```

So how do we make changes to our workflows to ensure we can achieve truly applied science? 
...
Before we present a formal definition, let's describe a modificaton of the conventional workflow that includes an open process to discovery and implementation [@Beck20; modified from @Hampton15]. 
...
1. __Open data__
     ...
     * Resuability and transparent workflows
     * Data provenance and metadat
...
1. __Open products__
     ...
     * Integration wiht external networks for discoverability

You'll notice that web-based tools and open science are often discussed at the same time.  Science existed before the internet and open science often focuses on how the two can leverage and support one another despite the latter being a relatively new addition to society.  
...
@Fecher14 describe open science as five distinct schools of thought: 

```{r}
knitr::include_graphics('img/fiveschools.PNG')
```
